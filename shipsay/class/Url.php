...
class Url
{
    ...
    {
        global $remote_img_url, $local_img, $is_multiple, $site_url;
        if (in_array($imgflag, [9, 1, 75])) {
    ...
    {
        $fake_url = $is_langtail ? $GLOBALS['fake_langtail_info'] : $GLOBALS['fake_info_url'];
        if (strpos($fake_url, '{subaid}')!== false) {
            $subaid = intval($aid / 1000);
            return str_replace(['{aid}', '{subaid}'], [$aid, $subaid], $fake_url);
        }
        return str_replace(['{aid}', '{acode}'], $aid, $fake_url);
    }

    static function index_url($aid, $pid = 1, $is_langtail = false)
    {
        $fake_url = $is_langtail ? $GLOBALS['fake_langtail_indexlist'] : $GLOBALS['fake_indexlist'];
        if (strpos($fake_url, '{subaid}')!== false) {
        ...
        return preg_replace($from, $to, $fake_url);
    }
    ...
    static function chapter_url($aid, $cid, $pid = 1)
    {
        ...
        if (strpos($fake_chapter_url, '{subaid}')!== false) {
            $subaid = intval($aid / 1000);
            if ($pid === 1) {
                return str_replace(['{subaid}', '{aid}', '{cid}'], [$subaid, $aid, $cid], $fake_chapter_url);
            }
            return str_replace(['{subaid}', '{aid}', '{cid}'], [$subaid, $aid, $cid . '_' . $pid], $fake_chapter_url);
        }
        if ($pid === 1) {
            return str_replace(['{aid}', '{acode}', '{cid}'], [$aid, $aid, $cid], $fake_chapter_url);
        }
        return str_replace(['{aid}', '{acode}', '{cid}'], [$aid, $aid, $cid . '_' . $pid], $fake_chapter_url);
    }
    ...
    static function author_url($author)
    {
        ...
        return '/author/' . urlencode($author) . '/';
    }
    ...
    static function category_url($sortid_or_sortcode, $pid = 1)
    {
        ...
        return str_replace($from, $to, $fake_sort_url);
    }
